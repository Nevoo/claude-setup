...
### JSON-LD Templates (geo/schema/)

Ready-to-paste JSON-LD templates live in `../geo/schema/` (`~/.claude/skills/geo/schema/`). Load the relevant one, replace the placeholder values, and generate from it instead of hand-writing markup:

| File | Primary @type | Use for |
|---|---|---|
| `organization.json` | Organization | Any business site — entity identity with full `sameAs` + `knowsAbout` |
| `article-author.json` | Article + Person (author) | Publishers / blog posts — E-E-A-T author signals |
| `local-business.json` | LocalBusiness | Businesses with physical locations (address, geo, hours) |
| `product-ecommerce.json` | Product | E-commerce product pages (offers, brand, ratings) |
| `software-saas.json` | SoftwareApplication | SaaS products (pricing offers, ratings) |
| `website-searchaction.json` | WebSite + SearchAction | Homepage sitelinks search box |

Follow the Generation Rules above when adapting a template (@graph pattern, absolute URLs, `@id` cross-references, ISO 8601 dates, `speakable` on Article, server-rendered placement).
