...
For an automated first pass, run `python3 ~/.claude/skills/geo/scripts/citability_scorer.py <url>` — it fetches the page, segments it into content blocks, and returns per-block citability scores. Use its output as a starting point, then refine with the rubric below.
...
> These are the canonical citability sub-weights (30/25/20/15/10), also recorded in `../geo/references/scoring.md`. The stale `geo-ai-visibility` agent used 25/20/20/20/15 — the skill weights above win.
