...
Check robots.txt for directives targeting the AI crawlers in the canonical roster: **`../geo/references/crawlers.md`** (`~/.claude/skills/geo/references/crawlers.md` — full per-crawler operators, user-agents, tiers, and platforms). Determine allow/block status for each.
...
Check HTTP response headers for the standard set — HSTS, CSP, X-Content-Type-Options, X-Frame-Options, Referrer-Policy, Permissions-Policy — and score per the rubric below.
