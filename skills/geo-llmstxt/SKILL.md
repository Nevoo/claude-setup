...
When creating a new llms.txt file from scratch, use the generator script. It handles site discovery (homepage, sitemap, navigation, business-type detection), page prioritization, description drafting, Key Facts extraction, and file assembly — following the specification above:

```bash
python3 ~/.claude/skills/geo/scripts/llmstxt_generator.py <url> generate
```

The script emits a JSON payload containing the generated `llms.txt` content plus discovery metadata (pages found, pages selected, prioritization rationale). The same script validates an existing file in `validate` mode:

```bash
python3 ~/.claude/skills/geo/scripts/llmstxt_generator.py <url> validate
```

### Finish and Validate the Generated File

Before outputting:
1. Verify all URLs are reachable (200 status).
2. Confirm total entry count is between 10-30.
3. Check that no description exceeds 50 words. Tighten any vague or marketing-heavy descriptions ("best," "leading," "revolutionary") into factual statements of what information is on the page.
4. Verify the overall file length is 50-150 lines.
5. Ensure Markdown formatting is clean and consistent, and that the file follows the specification and template above.
